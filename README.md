...
1. צור בוט ב-[@BotFather](https://t.me/BotFather) והעתק את הטוקן ל-`.env`
2. התקן תלויות: `pip install -r requirements.txt`
3. הפעל: `python telegram_bot.py`
4. שלח לבוט:

```
דודו אהרון | לילה טוב | Em,C,G,D
```

הבוט מחזיר את התמונה בצ'אט.

**זמר חדש מהטלפון:** שלח תמונה עם כיתוב `עידן עמדי` (או `/artist עידן עמדי` ואז תמונה). נשמר ב-`input/` אוטומטית.

אבטחה (מומלץ): שלח `/whoami`, העתק את ה-chat ID ל-`.env` → `TELEGRAM_ALLOWED_CHAT_IDS`.
